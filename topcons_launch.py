...
Usage:
topcons_launch.py [-h] -m {submit,get} [-seq FILE] [-jobname STR]
...
Example:
  ...
  python topcons_launch.py -m submit -seq COX3_gg.fasta
  
  # try to retrieve the result for jobid 'rst_TTT' and save it to the current directory
  python topcons_launch.py -m get -jobid rst_TTT -outpath output_topcons/
    ...
    if mode == "submit":
        ...
        if len(retValue) >= 1:
            ...
            if status == "Failed":
                ...
                urlretrieve(result_url, outfile)
